...
export default function HomeMedico() {
  ...
  useEffect(() => {
    ...
    if (!u || !u.grupos?.includes("medico_ubs")) { navigate("/"); return; }
    setUsuario(u);

    const ps: Paciente[] = JSON.parse(localStorage.getItem("pacientes") || "[]");
    setPacientes(ps);

    const consultas: any[] = JSON.parse(localStorage.getItem("consulta") || "[]");
    setRetornos(
      consultas
        .filter((c) => c.dataRetorno?.trim())
        .map((c) => ({
      ...
      <Dialog open={modalTipo !== null} onOpenChange={(open: boolean) => !open && fecharModal()}>
        <DialogContent className="p-0 gap-0 max-w-md rounded-2xl overflow-hidden">
          ...
          <div className="px-4 py-3 border-b border-slate-100 relative">
            <Search size={15} className="absolute left-7 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
            <Input
              autoFocus
            ...
            {filtrados.length === 0 ? (
              ...
              filtrados.map((p) => (
                <button
                  ...
                  className="flex items-center gap-3 w-full px-3.5 py-3 rounded-xl border border-slate-200 bg-white hover:bg-blue-50 hover:border-blue-200 transition-colors text-left"
                >
                  <div className="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center shrink-0">
                    <User size={16} className="text-slate-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-[14px] text-slate-900 leading-snug">{p.nome}</p>
                    <p className="text-xs text-slate-500 truncate mt-0.5">
                      CPF: {p.cpf} · {p.idade} anos · {p.suspeita}
                    </p>
                  </div>
                  <ArrowRight size={15} className="text-slate-400 shrink-0" />
                </button>
